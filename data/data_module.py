...
from .filter_gate import FilterGate
...
class DataCollatorForSeq2Seq:
    ...
    def __call__(self, features, return_tensors=None):
        ...
        for entity in entities:
            ...
            # print(self.entity_img_files)
            # print(en_file)
            en_imgs = []
            if en_file in self.entity_img_files:
                en_imgs = self.entity_img_files[en_file]
...
class KGC(BaseDataModule):
    def __init__(self, args, model) -> None:
        ...
        '''
        使用RSME的filter-gate.py过滤出6张图片
        '''
        # f = FilterGate(entity_img_path, hash_size=16)
        # f.filter()
        # if not os.path.exists('./dataset'):
        #     os.mkdir('./dataset/')
        # if not os.path.exists('./dataset/'):
        #     os.mkdir('./dataset/wn18-pickle/')
        # f.save_best_imgs('./data/wn18-pickle/wn18_best_img.pickle')
        file = open('./data/wn18-pickle/fb15k237_best_img.json')
        # wn18_best_img = pickle.load(file)
        wn18_best_img = json.load(file)
        # print(wn18_best_img.keys())
        file.close()

        # entity_img_files = list(wn18_best_img.values())
        ...
        self.sampler = DataCollatorForSeq2Seq(self.tokenizer,
            ...
            entity_img_files=wn18_best_img
