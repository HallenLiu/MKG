...
class FilterGate():
    ...
    #均值感知算法
    def ahash_sim(self,img1,img2,hash_size=None):
        if not hash_size:
            hash_size = self.hash_size

        img1_hash = imagehash.average_hash(Image.open(img1),hash_size=hash_size)
        img2_hash = imagehash.average_hash(Image.open(img2),hash_size=hash_size)

        return 1 - (img1_hash - img2_hash) / len(img1_hash) ** 2

    #差值感知算法
    def dhash_sim(self,img1,img2,hash_size=None):
        if not hash_size:
            hash_size = self.hash_size

        img1_hash = imagehash.dhash(Image.open(img1),hash_size=hash_size)
        img2_hash = imagehash.dhash(Image.open(img2),hash_size=hash_size)
